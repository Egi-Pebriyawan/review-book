<template>
        ...
        <div class="bg-white rounded-xl shadow-sm border p-6">
          ...
          <form @submit.prevent="submitComment" class="mb-6 p-4 bg-gray-50 rounded-lg space-y-4">
            ...
            <div>
              ...
              <textarea v-model="commentForm.content" required minlength="5" rows="4" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" placeholder="Tulis komentar Anda..."></textarea>
            </div>
            ...
            <p v-if="commentSubmitted" class="text-green-700 text-sm">
              ✓ Komentar Anda berhasil dikirim! Komentar akan tampil setelah disetujui.
            </p>
          </form>

          <!-- Comments List -->
          <div v-if="comments.length === 0" class="text-center py-8 text-gray-600">
  ...
  <div v-else-if="!pending" class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-20 text-center">
    ...
    <NuxtLink to="/books" class="px-6 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors">
...
useSeoMeta({
  title: `${slug} — Detail Review Buku`,
  description: `Baca review lengkap untuk buku ${slug}. Lihat rating, kelebihan, kekurangan, dan komentar pembaca.`
});
